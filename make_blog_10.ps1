...
# 1. Copy placeholder image if needed
$srcImg = Join-Path $baseDir "assets\blog9image1.webp"
$destImg = Join-Path $baseDir "assets\blog10image1.webp"
if (-not (Test-Path $destImg) -and (Test-Path $srcImg)) {
    Copy-Item $srcImg $destImg
    Write-Host "Created blog10image1.webp placeholder."
}

# 2. Build can-knee-arthritis-be-treated-without-surgery.html
$srcHtml = Join-Path $baseDir "bilateral-knee-replacement-pros-cons.html"
$destHtml = Join-Path $baseDir "can-knee-arthritis-be-treated-without-surgery.html"
...
$startSchemaMarker = '<!-- Structured Data (JSON-LD) for BlogPosting -->'
$endSchemaMarker = '<meta name="viewport"'

$startSchemaIdx = $content.IndexOf($startSchemaMarker)
$endSchemaIdx = $content.IndexOf($endSchemaMarker)

if ($startSchemaIdx -ne -1 -and $endSchemaIdx -ne -1) {
    $newSchemas = "<!-- Structured Data (JSON-LD) for BlogPosting -->`n  $blogPostingSchema`n`n  <!-- FAQ Schema -->`n  $faqSchema`n`n"
    $content = $content.Substring(0, $startSchemaIdx) + $newSchemas + $content.Substring($endSchemaIdx)
}
...
$tocHtml
              ...
              <!-- In-article Image 2: Knee Joint Osteoarthritis & Cartilage Degeneration -->
              <div class="article-image-wrap" style="width: 100%; margin: 36px 0; border-radius: var(--radius-lg); overflow: hidden; border: 1px solid var(--border); box-shadow: var(--shadow-sm);">
                <img src="assets/blog10image2.webp" alt="Knee Joint Osteoarthritis and Cartilage Degeneration" style="width: 100%; height: auto; display: block;" />
              </div>
              ...
              <!-- In-article Image 3: Knee Injections & Orthopaedic Treatment Procedure -->
              <div class="article-image-wrap" style="width: 100%; margin: 36px 0; border-radius: var(--radius-lg); overflow: hidden; border: 1px solid var(--border); box-shadow: var(--shadow-sm);">
                <img src="assets/blog10image3.webp" alt="Knee Injections and Orthopaedic Treatment Procedure" style="width: 100%; height: auto; display: block;" />
              </div>
              ...
              <!-- Direct Instagram Reel Video Player Embed -->
              <div class="video-embed-container" style="width: 100%; max-width: 480px; margin: 36px auto; border-radius: 16px; overflow: hidden; background: var(--bg-card); border: 1px solid var(--border); box-shadow: var(--shadow-sm);">
                <div style="padding: 16px 20px; background: rgba(225, 48, 108, 0.05); border-bottom: 1px solid var(--border);">
                  <h4 style="margin: 0; color: #E1306C; font-family: var(--font-head); font-size: 1.05rem; font-weight: 700; display: flex; align-items: center; gap: 8px;">
                    <span>🎥</span> Watch: Surgery Is NOT the Only Solution for Knee Arthritis
                  </h4>
                  <p style="margin: 6px 0 0; font-size: 0.88rem; color: var(--text-secondary); line-height: 1.5;">
                    Thinking about knee surgery because of persistent knee pain? In Episode 10, Dr. Madhuram Chowdry explains non-surgical treatment options and when knee surgery is actually necessary.
                  </p>
                </div>
                <div style="padding: 12px; background: var(--bg-card);">
                  <iframe src="https://www.instagram.com/reel/DYxD6I7z-E6/embed" width="100%" height="540" frameborder="0" scrolling="no" allowtransparency="true" style="width:100%; height:540px; border:none; border-radius:12px; display:block;" title="Surgery Is NOT the Only Solution for Knee Arthritis - Dr. Madhuram Chowdry Video Reel"></iframe>
                </div>
                <div style="padding: 10px 16px; background: var(--bg-card); text-align: center; border-top: 1px solid var(--border);">
                  <a href="https://www.instagram.com/reel/DYxD6I7z-E6/?utm_source=ig_web_copy_link&igsi=MzRlODBiNWFlZA==" target="_blank" rel="noopener noreferrer" style="display: inline-flex; align-items: center; gap: 6px; color: #E1306C; font-weight: 600; font-size: 0.84rem; text-decoration: none;">
              ...
              <div class="article-image-wrap" style="width: 100%; margin: 32px 0; border-radius: var(--radius-lg); overflow: hidden; border: 1px solid var(--border); box-shadow: var(--shadow-sm);">
                <img src="assets/blog10image4.png" alt="Dr. Madhuram Chowdry conducting orthopaedic knee arthritis consultation with patient" style="width: 100%; height: auto; display: block;" decoding="async" loading="lazy" />
              </div>
...
$startBodyIdx = $content.IndexOf('<div class="article-body">')
$endBodyIdx = $content.IndexOf('</article>', $startBodyIdx)

if ($startBodyIdx -ne -1 -and $endBodyIdx -ne -1) {
    $content = $content.Substring(0, $startBodyIdx) + $bodyHtml + $content.Substring($endBodyIdx)
}

[System.IO.File]::WriteAllText($destHtml, $content, [System.Text.Encoding]::UTF8)
Write-Host "Generated $destHtml successfully."

# 3. Update blog.html
$blogHtmlPath = Join-Path $baseDir "blog.html"
$blogPage = [System.IO.File]::ReadAllText($blogHtmlPath, [System.Text.Encoding]::UTF8)
...
$gridMarker = '<div class="blog-grid" id="blogGrid">'
if (-not $blogPage.Contains("can-knee-arthritis-be-treated-without-surgery.html") -and $blogPage.Contains($gridMarker)) {
    $blogPage = $blogPage.Replace($gridMarker, "$gridMarker`n$newBlogCard")
    [System.IO.File]::WriteAllText($blogHtmlPath, $blogPage, [System.Text.Encoding]::UTF8)
    Write-Host "Updated blog.html with new blog card."
}
...
if (-not $sitemap.Contains("can-knee-arthritis-be-treated-without-surgery.html") -and $sitemap.Contains("</urlset>")) {
    ...
    Write-Host "Updated sitemap.xml with new blog post link."
}
